...
# Cleanup of backups on remote region
resource "aws_lambda_function" "rds_snapshot_cleanup_remote" {
  count         = var.enable ? 1 : 0
  provider      = aws.replica
  function_name = "remove-snapshot-retention-${var.environment}"
  role          = aws_iam_role.iam_for_lambda[0].arn
...
resource "aws_iam_policy" "rds_cleanup_lambda_remote" {
  count    = var.enable ? 1 : 0
  provider = aws.replica
  name     = "rds-cleanup-lambda-remote-${var.environment}"

  policy = data.aws_iam_policy_document.rds_cleanup_lambda_remote.json
}

data "aws_iam_policy_document" "rds_cleanup_lambda_remote" {
  statement {
    actions = [
      "rds:DeleteDBSnapshot",
      "rds:Describe*",
    ...
    resources = ["arn:aws:rds:${var.aws_destination_region}::/snapshot/*"]
  }
}

resource "aws_iam_role_policy_attachment" "rds_cleanup_lambda_remote" {
  count      = var.enable ? 1 : 0
  provider   = aws.replica
...
resource "aws_cloudwatch_event_rule" "invoke_rds_cleanup_lambda_remote" {
  count               = var.enable ? 1 : 0
  provider            = aws.replica
  name                = "invoke-rds-cleanup-lambda-remote-${var.environment}"
  description         = "Fires every 24 hours"
  schedule_expression = "rate(24 hours)"
}

resource "aws_cloudwatch_event_target" "invoke_rds_cleanup_lambda_remote" {
  count     = var.enable ? 1 : 0
  provider  = aws.replica
  rule      = aws_cloudwatch_event_rule.invoke_rds_cleanup_lambda_remote[0].name
  target_id = "rds-backup"
  arn       = aws_lambda_function.rds_snapshot_cleanup_remote[0].arn
}

resource "aws_lambda_permission" "cloudwatch_invoke_rds_cleanup_lambda_remote" {
  count         = var.enable ? 1 : 0
  provider      = aws.replica
  statement_id  = "AllowExecutionFromCloudWatch"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.rds_snapshot_cleanup_remote[0].function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.invoke_rds_cleanup_lambda[0].arn
}
