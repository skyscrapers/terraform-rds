...
resource "aws_db_instance" "rds" {
  ...
  identifier                = "${length(var.name) == 0 ? "${var.project}-${var.environment}${var.tag}-rds${count.index}-replica" : var.name}"
  engine                    = "${var.engine}"
  ...
  tags {
    Name        = "${length(var.name) == 0 ? "${var.project}-${var.environment}${var.tag}-rds${count.index}-replica" : var.name}"
